...
export class ExamGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    private connectedAttempts = new Map<string, { socketId: string; disconnectTimer?: NodeJS.Timeout }>();
    ...
    async handleConnection(client: AuthenticatedSocket) {
        try {
            ...
            this.logger.warn(`JWT validation failed for ${client.id}: ${(err as Error).message}`);
        ...
        if (attemptId) {
            ...
            const timer = setTimeout(async () => {
                try {
                    ...
                    this.logger.error(`Error handling disconnect timeout: ${(err as Error).message}`);
                }
            }, 30_000);
