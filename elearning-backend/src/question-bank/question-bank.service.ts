  ...
  async bulkImportQuestions(
    ...
    if (!Array.isArray(questions) || questions.length === 0) {
      throw new BadRequestException(
        'Invalid input. Must be a non-empty array of questions.',
      );
    }
    ...
    const payload = questions.map((q, index) => {
      if (
        ...
        throw new BadRequestException(
          `Invalid question format at index ${index}. Must have content, at least 2 options, and correctAnswer index.`,
        );
      }
