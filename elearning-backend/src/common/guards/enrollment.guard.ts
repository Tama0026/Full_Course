  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!user) {
      throw new ForbiddenException(
        'Bạn vui lòng đăng nhập để thao tác và xem nội dung.',
      );
    }
    ...
    if (!courseId) {
      throw new ForbiddenException(
        'Unable to determine course for enrollment check',
      );
    }
    ...
    if (enrollment.status !== 'APPROVED') {
      throw new ForbiddenException(
        'Khóa học này yêu cầu sự phê duyệt từ giảng viên. Vui lòng chờ phê duyệt để xem nội dung.',
      );
    }
