...
export class GamificationResolver {
  ...
  async getTopStudents(
    ...
    return this.gamificationService.getTopStudents(
      limit,
  ...
  async getMyPoints(@CurrentUser() user: { id: string }): Promise<number> {
  ...
  @UseGuards(JwtAuthGuard)
  async getMyBadges(@CurrentUser() user: { id: string }): Promise<BadgeType[]> {
  ...
  async getCourseBadges(
    ...
    return this.gamificationService.getCourseBadges(
      courseId,
    ) as unknown as BadgeType[];
  }
  ...
  async getMyCreatedBadges(
    ...
    if (user.role !== 'INSTRUCTOR' && user.role !== 'ADMIN') {
      throw new ForbiddenException(
        'Chỉ Instructor mới có thể xem danh sách Badge đã tạo',
      );
    }
    return this.gamificationService.getInstructorBadges(
      user.id,
    ) as unknown as BadgeType[];
  }
  ...
  async updateCourseBadge(
    ...
    return this.gamificationService.updateCourseBadge(
      badgeId,
      user.id,
      input,
    ) as unknown as BadgeType;
  ...
  async adminUpdateBadge(
    ...
    return this.gamificationService.adminUpdateBadge(
      badgeId,
      input,
    ) as unknown as AdminBadgeType;
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.ADMIN)
  async adminDeleteBadge(@Args('badgeId') badgeId: string): Promise<boolean> {
  ...
  async getMyLoginStreak(
    ...
    return this.gamificationService.getLoginStreak(
      user.id,
    ) as unknown as LoginStreakType;
  }
}
