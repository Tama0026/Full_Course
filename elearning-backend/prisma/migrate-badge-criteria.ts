/**
 * migrate-badge-criteria.ts
 * 
 * One-time script to backfill existing badges' criteriaType + threshold
 * from the legacy `criteria` string.
 * 
 * Run: npx ts-node prisma/migrate-badge-criteria.ts
 */
...
const CRITERIA_MAP: Record<string, { criteriaType: string; threshold: number }> = {
...
async function main() {
    ...
    for (const badge of badges) {
        ...
        if (mapping) {
            ...
            // Try to auto-parse from criteria string pattern
